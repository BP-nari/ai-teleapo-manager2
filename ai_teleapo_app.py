...
class TeleapoDataManager:
    ...
    def merge_with_original(self, call_results_df, job_id):
        ...
        # 社名列を保持し、顧客名【コピー用】列も含める
        if has_call_time and '架電日' in merged_df.columns and '架電時間' in merged_df.columns:
            column_order = ['IDの頭にID', '社名', '顧客名【コピー用】', '電話番号', '架電日', '架電時間', 'ステータス', '架電結果', '要約', '通話時間', 
                           '住所統合', '最終結果', '最終前回結果【改訂】', '社員名', '次回コール日', '最終履歴メモ【改訂】', 'row_key']
        elif has_call_time:
            # 分割できなかった場合は元の架電時刻を使用
            column_order = ['IDの頭にID', '社名', '顧客名【コピー用】', '電話番号', '架電時刻', 'ステータス', '架電結果', '要約', '通話時間', 
                           '住所統合', '最終結果', '最終前回結果【改訂】', '社員名', '次回コール日', '最終履歴メモ【改訂】', 'row_key']
        else:
            column_order = ['IDの頭にID', '社名', '顧客名【コピー用】', '電話番号', 'ステータス', '架電結果', '要約', '通話時間', 
                           '住所統合', '最終結果', '最終前回結果【改訂】', '社員名', '次回コール日', '最終履歴メモ【改訂】', 'row_key']
